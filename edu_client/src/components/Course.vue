        ...
        <div class="main">
            ...
            <div class="condition">
                <ul class="cate-list">
                    ...
                    <li @click="category=cate.id" v-for="cate in category_list" :key="cate.id"
                ...
                <div class="course-item" v-for="course in course_list" :key="course.id">
                    ...
                    <div class="course-info">
                        ...
                        <p class="teather-info">{{ course.teacher.name }} {{ course.teacher.title }}
                            <span>共{{course.lessons}}课时/{{course.lessons==course.pub_lessons?'更新完成':`已更新${course.pub_lessons}课时`}}</span></p>
...
export default {
    ...
    // 用于监听vue中某个值是否发生了改变
    watch: {
        // 监听分类id是否发生改变 一旦发生变化则代表用户要获取该分类下的课程
        category() {
            // 重新发起请求获取分类对应的课程
    ...
    methods: {
        ...
        // 获取分类信息
        get_category() {
            this.axios.get(this.$settings.HOST + 'course/course_category/').then(res => {
