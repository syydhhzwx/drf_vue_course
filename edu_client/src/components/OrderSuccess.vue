    ...
    <div class="success">
        ...
        <div class="main">
            ...
            <div class="order-info" v-for="(course, index) in course_list" :key="index">
                ...
                <p class="info3"><b>课程信息：</b><span>{{course.name}}</span></p>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            AliPayResult() {
                // 可以获取到路由后拼接的所有参数
                console.log(location.search, "22222")
                this.axios.get(this.$settings.HOST + "pay/result/" + location.search).then(res => {
                    ...
                    console.log(res.data.real_price,res.data.course_list,res.data.pay_time);
                }).catch(error => {
                    // 如果支付失败，则不能停留在支付成功的页面
